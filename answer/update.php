<?php
    if ($_SERVER['REQUEST_METHOD'] === 'PUT') {
        ...
        if(!empty($data)) { // The state of the post body has been checked.
            ...
            $db_answer = $db->query("SELECT * FROM answers WHERE uid = '$uid' AND top = '$top' AND coin = '$coin' AND date = '$date' AND time = '$time'")->fetch(PDO::FETCH_ASSOC);
            ...
            if(!empty($db_answer)) { // The status of the information has been checked.
                ...
                $run = $db->prepare("UPDATE answers SET comment = ? WHERE uid = '$uid' AND name = '$row[name]' AND top = '$top' AND profile = '$row[profile]' AND coin = '$coin' AND date ='$date' AND time ='$time'");
